...
```{r, message = FALSE}
run.gadgets(pp.list, n.chromosomes = c(4,5), chromosome.size = 3, 
       results.dir = "size3_res", cluster.type = "interactive",
       registryargs = list(file.dir = "size3_reg", seed = 1300),
       n.islands = 20, island.cluster.size = 4, 
       n.migrations = c(1, 2), n.different.snps.weight = c(2, 1, 2, 1, 1), 
       n.both.one.weight = c(1, 0, 1, 0, 0), weight.function.int = c(2, 2, 1, 1, 0))

run.gadgets(pp.list, n.chromosomes = c(4,5), chromosome.size = 4, 
...
```{r}
size3.best.weights <- choose.best.weights("size3_res", pp.list)
size4.best.weights <- choose.best.weights("size4_res", pp.list)
...
lapply(chrom.sizes, function(chrom.size){
  ...
  if (chrom.size == 3){
    ...
    best.weights <- size3.best.weights$family.weights
    n.dif.weight <- best.weights["n.different.snps.weight"]
    n.both.one.weight <- best.weights["n.both.one.weight"]
    weight.int <- best.weights["weight.function.int"]
    recessive.prop <- size3.best.weights$sampled.params$recessive.ref.prop
    
  } else {
    
    best.weights <- size4.best.weights$family.weights
    n.dif.weight <- best.weights["n.different.snps.weight"]
    n.both.one.weight <- best.weights["n.both.one.weight"]
    weight.int <- best.weights["weight.function.int"]
