...
We start by computing the SNP-pair scores using `compute.pair.scores`. This function takes as arguments a list of data.tables containing the results from GADGETS run for different chromosome sizes, the list of pre-processed data from function `preprocess.genetic.data`, and the number of top scoring chromosomes to use in constructing the plot (here we use 5 for illustration, in practice we use 50).  

```{r}
set.seed(10)
obs.res.list <- list(size3.combined.res[1:5, ], size4.combined.res[1:5, ])
