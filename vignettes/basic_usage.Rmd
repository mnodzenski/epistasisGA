...
vignette: >
  %\VignetteIndexEntry{GADGETS Usage}
...
Users may also experience an error when executing `epistasis.test` that says "cannot run test, all SNPs are in LD". This is generated because the procedure can only run if at least two SNPs are located in different LD blocks, otherwise other methods (not provided in this package) are required. 
...
network.plot(pair.scores, pp.list, graph.area = 200,
             node.size = 40, vertex.label.cex = 2)
```

We see the true pathway SNPs appear prominently in this plot. Key features of the plotting approach are (1) the thickness of the SNP-to-SNP line segments is proportional to the log of the graphical score for that pair and (2) the area of each node circle is proportional to the log of the sum of the graphical scores for all pairs that include the SNP. Although not seen in the plot above, dashed connections indicate SNPs in high LD (pairwise $r^2 \geq 0.1$ by default) in the unaffected siblings/complements. 

As a more minor point, the SNP labels default to the RSIDs provided, as seen in the third and fourth columns of the `pair.scores` data frame:
```{r}
head(pair.scores)
```

However, if users desire custom labels, they can do so by changing the values in the third and fourth columns. For instance, here we label the true risk SNPs with a star and leave all others blank:

```{r, fig.width = 14, fig.height = 12}
risk.numbers <- c(51, 52, 76, 77)
pair.scores$SNP1.rsid <- ifelse(pair.scores$SNP1 %in% risk.numbers, "*", "")
pair.scores$SNP2.rsid <- ifelse(pair.scores$SNP2 %in% risk.numbers, "*", "")
network.plot(pair.scores, pp.list, graph.area = 200,
