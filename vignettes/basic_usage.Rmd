...
```{r}
set.seed(1400)
perm.data.list <- permute.dataset(pp.list, 
...
```{r}
preprocess.lists <- lapply(seq_along(perm.data.list), function(permutation){
  
  perm.data.desc <- perm.data.list[[permutation]]
  perm.data.path <- paste0("bm_tmp_p", permutation)
  perm.case <- bigmemory::attach.big.matrix(perm.data.desc$case)
  perm.mom <- bigmemory::attach.big.matrix(perm.data.desc$mother)
  perm.dad <- bigmemory::attach.big.matrix(perm.data.desc$father)
  
  preprocess.genetic.data(perm.case, father.genetic.data = perm.dad,
                          mother.genetic.data = perm.mom, 
                          ld.block.vec = ld.block.vec, 
                          big.matrix.file.path = perm.data.path)
})
