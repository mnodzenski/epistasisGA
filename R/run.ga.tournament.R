...
#' @param tournament.size The number of tournament participants in tournament selection of parents for the next generation. Defaults to 3.
...
run.ga.tournament <- function(case.genetic.data, complement.genetic.data = NULL, father.genetic.data = NULL, mother.genetic.data = NULL,
                   ...
                   snp.sampling.type = "zscore", warmup.gens = 0, crossover.prop = 0.5, tournament.size = 3){
  ...
  while (generation <= generations & !last.gens.equal){
    ...
    lower.idx <- which(sample.these)
    sampled.lower.idx <- rep(NA, length(lower.chromosomes))
    if (generation <= warmup.gens){

      for (i in 1:length(lower.chromosomes)){

        iter.sample <- sample(lower.idx, tournament.size)
        tournament.scores <- dif.vec.maxs[iter.sample]
        winner <- iter.sample[which.max(tournament.scores)]
        sampled.lower.idx[i] <- winner
      ...
      for (i in 1:length(lower.chromosomes)){
        ...
        iter.sample <- sample(lower.idx, tournament.size)
        tournament.scores <- fitness.scores[iter.sample]
        winner <- iter.sample[which.max(tournament.scores)]
        sampled.lower.idx[i] <- winner
