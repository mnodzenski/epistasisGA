...
#' @param chromosome.mat.list A list of data.tables containing all chromosomes from previous generations.
#' @param sum.dif.vec.list A list of data.tables containing the sum of differences vectors for chromosomes in previous generations.
#' @param risk.allele.vec.list A list of data.tables containing the number of risk alleles required for each SNP in each chromosome
#'  for a case or complement to be classified as having the full risk set, from previous generations.
...
#'  \item{chromosome.mat.list}{A list of data.tables containing the chromosomes examined in previous generations.}
#'  \item{sum.dif.vec.list}{A list of data.tables containing the sum of the difference vectors for all examined chromosomes.}
#'  \item{risk.allele.vec.list}{A list of data.tables containing the number of risk alleles required for each SNP in each chromosome
#'  for a case or complement to be classified as having the full risk set, from previous generations.}
...
evolve.island.original <- function(n.migrations = 20, case.genetic.data, complement.genetic.data, case.comp.different,
    ...
    if (generation == 1) {
        ...
        risk.allele.vec.list <- vector(mode = "list", length = max.generations)
