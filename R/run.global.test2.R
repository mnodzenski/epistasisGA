...
#'  \item{element.pvals}{A vector containing one-sided p-values comparing the KS-statistics
#'  in \code{obs.ks.vec} with \code{perm.ks.mat} for each of the d chromosome sizes.}
...
run.global.test2 <- function(results.list, n.top.scores = 1000) {
    ...
    chrom.size.ks.list <- lapply(results.list, function(chrom.size.res) {
        ...
        pos.perm.ks <- rowMaxs(mean.perm.ecdf.mat - perm.ecdf.mat)
        print(length(pos.perm.ks))
        neg.perm.ks <- rowMaxs(perm.ecdf.mat - mean.perm.ecdf.mat)
        perm.ks <- rep(NA, length(pos.perm.ks))
        for (i in seq_along(pos.perm.ks)){

            pos.val <- pos.perm.ks[i]
            neg.val <- neg.perm.ks[i]
            if (pos.val > neg.val){
    ...
    # take sums
    obs.sum <- sum(obs.ks.vec)
    perm.sums <- rowSums(perm.ks.mat)

    # count the number of permutations greater than the observed
    pval <- sum(perm.sums >= obs.sum)/length(perm.sums)

    # also look at element-wise results

    # element wise p-vals
    element.pvals <- vapply(seq_len(length(obs.ks.vec)), function(element.position) {

        obs.elem <- obs.ks.vec[element.position]
        perm.elems <- perm.ks.mat[, element.position]
    ...
    # return results list
    res.list <- list(obs.test.stat = obs.sum,  perm.test.stats = perm.sums, pval = pval,
                     obs.ks.vec = obs.ks.vec, perm.ks.mat = perm.ks.mat, element.pvals = element.pvals,
