...
#'  \item{obs.ad.vec}{A vector of observed Kolmogorov Smirnov test statistics for each chromosome size.}
#'  \item{perm.ad.mat}{A matrix of Kolmogorov Smirnov test statistics for the permutation datasets, where rows correspond to
...
run.global.test2 <- function(results.list) {

    # determine sample size
    ss <- min(unlist(lapply(results.list, function(chrom.size.res){

        obs.n <- length(chrom.size.res$observed.data)
        perm.n <- vapply(chrom.size.res$permutation.list, length, 1.0)
        min(c(obs.n, perm.n))

    })))

    # subset to sample size
    results.list <- lapply(results.list, function(chrom.size.res){

        obs.res <- sort(chrom.size.res$observed.data, decreasing = TRUE)[seq_len(ss)]
        perm.res.list <- chrom.size.res$permutation.list
        perm.res.list <- lapply(perm.res.list, function(y) sort(y, decreasing = TRUE)[seq_len(ss)])
        list(observed.data = obs.res, permutation.list = perm.res.list)

    })

    # loop over chromosome sizes
    chrom.size.ad.list <- lapply(results.list, function(chrom.size.res) {
        ...
        obs.fitness.scores <- sort(chrom.size.res$observed.data)

        # grab the permuted data
        perm.list <- lapply(chrom.size.res$permutation.list, sort)
        ...
        # now get the anderson-darling test stat compared with the mean eCDF
        obs.ad <- -1*sum(vapply(seq_len(ss), function(i){

            y.i <- obs.fitness.scores[i]
            y.n.plus.1.minus.i <- obs.fitness.scores[ss + 1 - i]

            f.yi <- mean.perm.ecdf[all.fitness.scores == y.i]
            if (f.yi == 0) f.yi <- 10^-10
            f.y.np1.minus.i <- mean.perm.ecdf[all.fitness.scores == y.n.plus.1.minus.i]
            if (f.y.np1.minus.i == 1) f.y.np1.minus.i <- 1 - 10^-10
            ((2*i - 1)/ss)*(log(f.yi) + log(1 - f.y.np1.minus.i))

        }, 1.0))
        perm.ad <- vapply(chrom.size.res$permutation.list, function(perm.scores){

            -1*sum(vapply(seq_len(ss), function(i){

                y.i <- perm.scores[i]
                y.n.plus.1.minus.i <- perm.scores[ss + 1 - i]
        ...
        return(list(obs.ad = obs.ad, perm.ad = perm.ad, mean.perm.ecdf = mean.perm.ecdf))

    })

    # store the mean permutation based ecdfs
    mean.ecdf.list <- lapply(chrom.size.ad.list, function(x) x$mean.perm.ecdf)

    # grab the observed vector
    obs.ad.vec <- vapply(chrom.size.ad.list, function(x) x$obs.ad, 1.0)

    # matrix of permutation based vectors
    perm.ad.mat <- vapply(chrom.size.ad.list, function(x) x$perm.ad,
                          rep(1.0, length(chrom.size.ad.list[[1]]$perm.ad)))

    # mean permutation based vector
    mean.perm.vec <- colMeans(perm.ad.mat)

    # covariance for permutation based vector
    perm.cov.mat <- cov(perm.ad.mat)
    perm.cov.mat.inv <- solve(perm.cov.mat)

    # calculate mahalanobis distance to mean vector for obs data
    obs.mean.dif <- obs.ad.vec - mean.perm.vec
    ...
    perm.mahala <- rep(NA, nrow(perm.ad.mat))
    for (i in seq_len(nrow(perm.ad.mat))) {

        perm.ad.vec <- perm.ad.mat[i, ]
        perm.mean.dif <- perm.ad.vec - mean.perm.vec
    ...
    obs.elements <- as.numeric((obs.ad.vec - mean.perm.vec)/diag(perm.cov.mat.inv))^2
    perm.elem.mat <- matrix(NA, nrow(perm.ad.mat), ncol = ncol(perm.ad.mat))
    for (i in seq_len(nrow(perm.ad.mat))) {

        perm.ad.vec <- perm.ad.mat[i, ]
        perm.elem.mat[i, ] <- ((perm.ad.vec - mean.perm.vec)/diag(perm.cov.mat.inv))^2
    ...
    res.list <- list(obs.test.stat = obs.mahala, pval = pval, perm.test.stats = perm.mahala, element.test.stats = obs.elements,
        element.pvals = element.pvals, perm.elem.test.stat.mat = perm.elem.mat, obs.ad.vec = obs.ad.vec,
        perm.ad.mat = perm.ad.mat, max.obs.fitness = max.obs.fitness, max.perm.fitness = max.perm.fitness,
        max.order.pvals = max.order.pvals, mean.ecdf.list = mean.ecdf.list)
    return(res.list)
