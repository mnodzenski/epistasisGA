...
chrom.fitness.score2 <- function(case.genetic.data, complement.genetic.data, case.comp.differences,
  ...
  ### expand input data based on weights ###
  w.case.inf <- case.inf[ , rep(seq_len(ncol(case.inf)), family.weights)]
  w.comp.inf <- comp.inf[ , rep(seq_len(ncol(case.inf)), family.weights)]
  genetic.df <- as.data.frame(t(cbind(w.case.inf, w.comp.inf)))
  pred.vars <- colnames(genetic.df)

  ### run conditional logistic model on augmented data ###
  genetic.df$case.status <- c(rep(1, ncol(w.case.inf)), rep(0, ncol(w.comp.inf)))
  genetic.df$id <- rep(seq_len(ncol(w.case.inf)), 2)
  model.form <- as.formula(paste0("case.status ~ ", paste(pred.vars, collapse = "+"),
                       "+ strata(id)"))
  cond.res <- clogit(model.form, data = genetic.df, method = "approximate")

  ### get test stat ###
  test.stat <- summary(cond.res)$logtest[1]

  ### get information on the estimated coefficients ###
  coefs <- summary(cond.res)$coef[ , "z"]

  ### determine how many cases and complements actually have the proposed risk set ###
  risk.dirs <- sign(coefs)
  pos.risk <- which(risk.dirs > 0)
  neg.risk <- which(risk.dirs <= 0)

  n.target <- length(target.snps)
  case.high.risk <- (colSums(case.inf[pos.risk, , drop = FALSE] > 0) + colSums(case.inf[neg.risk, ,
  ...
  case.high.risk.alleles <- colSums(case.high.inf[pos.risk, , drop = FALSE]) + (colSums(2 - case.high.inf[neg.risk,
                                                                                                          , drop = FALSE]))
  total.case.high.risk.alleles <- sum(case.high.risk.alleles)
  comp.high.risk.alleles <- colSums(comp.high.inf[pos.risk, , drop = FALSE]) + (colSums(2 - comp.high.inf[neg.risk,
                                                                                                          , drop = FALSE]))
  ...
  fitness.score <- test.stat

  return(list(fitness.score = fitness.score, coefs = coefs, rr = rr, test.stat = test.stat,
