...
evolve.island <- function(n.migrations = 20, case.genetic.data, complement.genetic.data, case.comp.different,
                          ...
                          snp.chisq, original.col.numbers, all.converged = FALSE,
  ...
  while (generation <= generations & !all.converged){
    ...
    sampled.lower.idx <- sample(which(sample.these), length(lower.chromosomes),
                                replace = TRUE, prob = fitness.scores[sample.these])
    ...
    if (round(length(unique.lower.idx)*crossover.prop) %% 2 == 0 | length(unique.lower.idx) == 1){
      ...
      cross.overs[sample(seq_len(length(unique.lower.idx)), size = round(length(unique.lower.idx)*crossover.prop))] <- TRUE

    } else {

      cross.overs[sample(seq_len(length(unique.lower.idx)), size = (round(length(unique.lower.idx)*crossover.prop) + 1))] <- TRUE
    ...
    if (sum(cross.overs) > 1){
      ...
      for (i in cross.over.starts){
        ...
        if (chrom1.fitness.score >= chrom2.fitness.score){
          ...
          c2.c1.not.matching.snp.positions <- c2.c1.not.matching.snp.positions[order(abs(chrom1.dif.vecs[c2.c1.not.matching.snp.positions]), decreasing = TRUE)]

        } else{

          c1.c2.not.matching.snp.positions <- c1.c2.not.matching.snp.positions[order(abs(chrom2.dif.vecs[c1.c2.not.matching.snp.positions]), decreasing = TRUE)]
    ...
    snps.for.mutation <- sample(seq_len(n.candidate.snps), n.candidate.snps, prob = snp.chisq, replace = TRUE)
  ...
  if (generation < max.generations & ! all.converged & !is.null(n.migrations)){
    ...
    colnames(unique.chrom.dif.vec.dt) <- paste0("snp", seq_len(ncol(unique.chrom.dif.vec.dt)), ".diff.vec")
