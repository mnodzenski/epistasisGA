...
#' @param type The approach to computing the global test.
...
#'
#'  ## run global test
#'  global.test.res <- run.global.test3(final.results)
#'
...
run.global.test3 <- function(results.list, n.top.scores = 10, type = "ranks") {
    ...
    chrom.size.ranks <- do.call(cbind, lapply(results.list, function(chrom.size.res) {
        ...
        if (type == "ranks"){
            ...
            # return ranks
            return(data.frame(score = score.ranks))

        } else if (type == "sums"){

            return(data.frame(score = all.scores))
