...
#'
#' data(cases)
#' fitness.score.cpp(target.snps = c(1, 4, 7), case.genetic.data = cases)
#'
#' @importFrom stats dist
#' @useDynLib knnGA
#' @export

fitness.score.cpp <- function(case.genetic.data, target.snps, k = 10, correct.thresh = 0.9){
  ...
  ids <- rownames(cases)
  prop.neighbor.case.by.id <- knn_prop_cases(n.informative.families, distances, k, correct.thresh,
                                             as.numeric(rownames(case.comp.data)),
                                             rep(1, (2*n.informative.families)))
